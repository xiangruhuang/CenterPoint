...
class KalmanGroupTracker(object):
    ...
    def track_one_step(self, selected_graphs, frame_id, temporal_dir):
    ...
    def track(self, graph_idx, _velocity):
        velocity = _velocity.clone().cuda()
        num_graphs = graph_idx.max().item()+1
        current_center = torch.zeros(num_graphs, 3, dtype=torch.float32).cuda()
        last_center = torch.zeros_like(current_center)
        second_last_center = torch.zeros_like(last_center)

        graph_frame_id = scatter(self.points[:, -1], graph_idx, dim=0,
                                 dim_size=num_graphs, reduce='max').long()

        # forward
        for frame_id in range(0, num_frames):
            graph_mask = graph_frame_idx == frame_id
            selected_graphs = graph_idx[graph_mask]
